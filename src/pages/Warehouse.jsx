...
function FormField({ label, placeholder, multiline = false }) {
  const inputStyles =
    "w-full rounded-2xl border border-slate-200 bg-white/50 px-4 py-3 text-base text-slate-800 placeholder:text-slate-400 focus:border-slate-400 focus:outline-none focus:ring-0 dark:border-slate-800 dark:bg-slate-900/70 dark:text-slate-100 dark:placeholder:text-slate-500";
...
export default function Warehouse() {
  return (
    <main className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-10">
      <p className="text-center text-sm tracking-[0.2em] uppercase text-slate-400 dark:text-slate-500">
      ...
      <section className="rounded-[32px] border border-white/60 bg-white/80 p-8 shadow-xl shadow-slate-200/60 backdrop-blur dark:border-slate-800/60 dark:bg-slate-950/70 dark:shadow-black/30">
        ...
        <div className="mt-8 grid gap-6 md:grid-cols-2">
          <FormField label="Name" placeholder="Main Distribution Center" />
          <FormField label="Short Code" placeholder="MDC" />
          <div className="md:col-span-2">
            <FormField
              label="Address"
              placeholder="Add street, city, and country details"
              multiline
            />
          </div>
        </div>
        ...
        <p className="mt-10 text-sm text-slate-400 dark:text-slate-500">
          Capture key identifiers for each warehouse to keep operations, labels,
          and documentation perfectly in sync.
        </p>
      </section>
    </main>
  );
